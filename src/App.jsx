...
const FLAGS = {
  ...
  England: "🏴", Netherlands: "🇳🇱", Serbia: "🇷🇸", Iran: "🇮🇷",
...
const styles = {
  ...
  notification: { position: "fixed", top: 20, right: 20, zIndex: 9999, padding: "10px 16px", borderRadius: 8, color: "#fff", display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 600, boxShadow: "0 4px 20px rgba(0,0,0,0.4)" },
...
export default function App() {
  ...
  if (!currentUser) {
    return <LoginScreen form={loginForm} setForm={setLoginForm} onLogin={handleLogin} error={loginError} users={users} />;
  }
    ...
    <div style={styles.app}>
      ...
      <header style={styles.header}>
        ...
        <nav style={styles.nav}>
          {navItems.map(item => (
            <button key={item.id} style={{ ...styles.navBtn, ...(view === item.id ? styles.navBtnActive : {}) }} onClick={() => setView(item.id)}>
    ...
    <div style={styles.loginPage}>
      ...
      <div style={styles.loginCard}>
        ...
        <div style={styles.loginForm}>
          <input style={styles.input} type="email" placeholder="Email address" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} onKeyDown={e => e.key === "Enter" && onLogin()} />
          <input style={styles.input} type="password" placeholder="Password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} onKeyDown={e => e.key === "Enter" && onLogin()} />
...
function LeaderboardView({ users, scores, matches }) {
  ...
  const ranked = [...users].map(u => ({ ...u, ...scores[u.id] })).sort((a, b) => b.total - a.total);
  const medals = ["🥇", "🥈", "🥉"];
...
function PredictionsView({ user, matches, predictions, setPredictions, notify, scores }) {
  ...
  const grouped = Object.entries(WC2026_GROUPS).map(([group, teams]) => ({ group, matches: matches.filter(m => m.group === group) }));
  const filteredGroups = filter === "all" ? grouped : filter === "unpredicted" ? grouped.map(g => ({ ...g, matches: g.matches.filter(m => !userPreds.find(p => p.matchId === m.id)) })).filter(g => g.matches.length > 0) : grouped.filter(g => g.group === filter);

  const savePrediction = () => {
    const h = parseInt(predForm.home), a = parseInt(predForm.away);
    if (isNaN(h) || isNaN(a) || h < 0 || a < 0) {
      notify("Enter valid scores (0+)", "error");
      return;
    }
          ...
          <div style={styles.modalCard}>
            ...
            <div style={styles.scoreInputRow}>
              <div style={styles.scoreInputGroup}>
                ...
                <input style={styles.scoreInput} type="number" min="0" max="20" value={predForm.home} onChange={e => setPredForm(f => ({ ...f, home: e.target.value }))} autoFocus />
              </div>
              <span style={styles.scoreDash}>–</span>
              <div style={styles.scoreInputGroup}>
                <label style={styles.scoreLabel}>{editingMatch.away}</label>
                <input style={styles.scoreInput} type="number" min="0" max="20" value={predForm.away} onChange={e => setPredForm(f => ({ ...f, away: e.target.value }))} />
              </div>
            </div>
...
function MatchesView({ matches, predictions, currentUser, scores }) {
  ...
  const filtered = filter === "played" ? matches.filter(m => m.result) : filter === "upcoming" ? matches.filter(m => !m.result) : matches;
...
function AnalyticsView({ users, matches, predictions, scores }) {
  ...
  const ranked = [...users].map(u => {
    ...
    const accuracy = totalPred > 0 && played.length > 0 ? Math.round(((s.correct || 0) / Math.min(totalPred, played.length)) * 100) : 0;
